                    ...
                    for (int i = 0; i < numButtons; ++i) {
                        if (CheckButton(&(SDL_Point){mouseCordsX, mouseCordsY}, &buttonArray[i]->rect)) {
                            if (!buttonArray[i]->playedSound) { 
                                Mix_PlayChannel(-1, buttonSound, 1); // Відтворюємо звук без повторення
                                buttonArray[i]->playedSound = true; // Встановлюємо прапорець, що звук вже був відтворений
                            }
...
// int ChooseDifficulty(SDL_Renderer* renderer) {
//     Entity* defaultScreen = CreateEntity(renderer, 0, 0, SCREENWIDTH, SCREENHEIGHT, "resource/images/defaultScreen.png");
//     Entity* hardScreen = CreateEntity(renderer, 0, 0, SCREENWIDTH, SCREENHEIGHT, "resource/images/hardScreen.png");
//     Entity* mediumScreen = CreateEntity(renderer, 0, 0, SCREENWIDTH, SCREENHEIGHT, "resource/images/mediumScreen.png");
//     Entity* babyScreen = CreateEntity(renderer, 0, 0, SCREENWIDTH, SCREENHEIGHT, "resource/images/babyScreen.png");
...
//                     for (int i = 0; i < numButtons; ++i) {
//                         if (CheckButton(&(SDL_Point){mouseX, mouseY}, &buttonArray[i]->rect)) {
//                             indexScreen = i + 1; // Set index for the screen based on hovered button
//                             buttonArray[i]->state = STATE2;
//                             buttonHovered = true;
//                         } else {
//                             buttonArray[i]->state = STATE1;
//                         }
//                     }
//                     // If no button is hovered, set default screen index
//                     if (!buttonHovered) {
//                         indexScreen = 0;
//                     }
//                     break;
//             }
//         }
//         ShowEntity(renderer, backgroundArray[indexScreen]);
//         for (int i = 0; i < numButtons; i++) {
//             ShowButton(renderer, buttonArray[i]);
//         }
//         SDL_RenderPresent(renderer);
...
//     // Clean up entities and buttons
//     for (int i = 0; i < sizeof(backgroundArray) / sizeof(backgroundArray[0]); i++) {
//         DestroyEntity(backgroundArray[i]);
//     }
